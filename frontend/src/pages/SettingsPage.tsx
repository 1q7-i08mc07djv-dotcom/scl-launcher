...
const DOWNLOAD_SOURCES = [
  { key: 'BMCLAPI', label: 'BMCLAPI', desc: '速度快，无需 Token' },
  { key: 'MCBBS', label: 'MCBBS', desc: '可选' },
  { key: 'Aliyun', label: '阿里云', desc: '速度快，无需 Token' },
  { key: 'Tencent', label: '腾讯云', desc: '速度较好' },
  { key: 'GitCode', label: 'GitCode', desc: '已配置 Token' },
];
...
export default function SettingsPage() {
  ...
  const loadConfig = useCallback(async () => {
    try {
      ...
      setJvmArgs(cfg.jvmArgs);
      setGitcodeToken(cfg.gitcodeToken);
      setAutoJava(cfg.autoJava);
      setDownloadSource(cfg.downloadSource);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Sync to store
      setJavaPath(localJavaPath);
      setMemory(`${localMemory}G`);
      setJvmArgs(localJvmArgs);
      setGitcodeToken(localGitcodeToken);
      setAutoJava(localAutoJava);
      setDownloadSource(localDownloadSource);
  ...
  return (
    <div className="flex w-full h-full" style={{ backgroundColor: '#1A1A1A' }}>
      ...
      <div
        ...
        style={{ width: 180, backgroundColor: '#1E1E1A' }}
      >
        <div className="p-4 border-b border-pcl-border">
          <h3 className="text-sm font-medium text-white/60">{t('settings.title')}</h3>
        </div>
                        ...
                        <button
                          ...
                          className={`
                            ...
                            ${localLanguage === lang.key
                              ...
                              : 'bg-pcl-card border-pcl-border text-white/60 hover:text-white'
                            }
                          `}
                        >
                          {lang.label}
                    ...
                    <div className="space-y-2">
                      {DOWNLOAD_SOURCES.map((source) => (
                        <button
                          ...
                          className={`
                            w-full text-left px-3 py-2.5 rounded text-sm transition-colors border
                            ${localDownloadSource === source.key
                              ? 'bg-pcl-highlight/20 border-pcl-highlight text-white'
                              : 'bg-pcl-card border-pcl-border text-white/60 hover:text-white'
                            }
                          `}
                        >
                          ...
                          <div className="text-xs text-white/40 mt-0.5">{source.desc}</div>
                        </button>
                      ))}
                    </div>
                  </MyCard>
                ...
                <div className="space-y-4">
                  <MyCard title={t('settings.theme')}>
                    <div className="flex gap-3">
                      <button
                        className="flex-1 py-3 rounded border text-sm bg-pcl-card border-pcl-highlight text-white"
                      >
                        🌙 {t('settings.dark')}
                      </button>
                      <button
                        disabled
                        className="flex-1 py-3 rounded border text-sm bg-pcl-card border-pcl-border text-white/30 cursor-not-allowed"
                      >
                        ☀️ 浅色（敬请期待）
                      </button>
                    </div>
